...
# Download modelnya kalo belom ada
...
# Buka kamera buat nangkep video
cap = cv2.VideoCapture(0)

# Daftar warna pake format BGR
colors = [(255, 0, 0), (0, 255, 0), (0, 0, 255), (0, 255, 255)] # Biru, Ijo, Merah, Kuning
...
while True:
    ...
    frame = cv2.flip(frame, 1) # Balik layarnya biar kayak cermin
    framergb = cv2.cvtColor(frame, cv2.COLOR_BGR2RGB)
    
    # Gambar tombol-tombol UI di layar
    ...
    # Lacak tangan pake Tasks API yang baru
    ...
    if result and len(result.hand_landmarks) > 0:
        ...
        for i, hand_landmarks in enumerate(result.hand_landmarks):
            ...
            # Ngitung kordinat posisi jari di layar
            landmarks = []
            ...
            # Ngecek tangan lagi ngebuka alias mode ngapus
            ...
            if fingers_open == 0:
                # Kalo tangannya ngepal -> Mode ngapus
                if palm_center[1] > 125: # Sisain ruang diatas biar tetep bisa klik UI
                    cv2.circle(frame, palm_center, 40, (122, 122, 122), 2) # Bikin buletan abu-abu buat efek UI pas ngapus
                ...
                # Telunjuk doang yang naik -> Cek lagi nyentuh UI atau lagi nggambar
                if center[1] <= 65:
                    if 40 <= center[0] <= 140: # Tombol buat hapus semua
                    ...
                    # Ngatur ukuran brush dari tombol UI
                    if state.cooldown == 0:
                        if 40 <= center[0] <= 140:
                            state.brushSize = min(50, state.brushSize + 1)
                            state.cooldown = 2 # Kasih jeda biar ukurannya ga cepet banget naiknya
                ...
                # Kalo gaya tangan lain -> Garis putus
                if len(state.strokes) > 0 and len(state.strokes[-1]['points']) > 0:
                    state.strokes.append({'color': colors[state.colorIndex], 'thickness': state.brushSize, 'points': deque(maxlen=512)})
                continue
            
        # Kalo tangannya ga ke-detect ya putus garisnya
        ...
        # Tangan ilang dari layar, putus garisnya
        for hand_category in ['Left', 'Right']:
            state = hands_state[hand_category]
            if len(state.strokes) > 0 and len(state.strokes[-1]['points']) > 0:
                state.strokes.append({'color': colors[state.colorIndex], 'thickness': state.brushSize, 'points': deque(maxlen=512)})

    # Tampilin ukuran brush yang lagi aktif di layar biar ketauan
    ...
    # Bikin kanvas kosong dulu buat tempat naruh coretan
    canvasForLines = np.zeros((480, 640, 3), dtype=np.uint8) + 255
    
    # Gambar coretan garis ke atas kanvasnya
    for hand_category in ['Left', 'Right']:
        state = hands_state[hand_category]
    ...
    # Bikin mask biar coretan garis di kanvas nyatu sama video di kamera
    ...
    # Bikin ulang paintWindow dari awal biar UI-nya selalu update bareng layarnya
    ...
    # Tombol buat ubah ukuran brush di paintWindow
    ...
    # Gabungin coretan yang udah digambar sama layout tombol paintWindow
    ...
    # Kalo dipencet tombol 'q' keluar aja dari program
